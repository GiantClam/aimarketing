...
async function requestPinned(url: URL, hostname: string, address: string, family: number) {
  const client = url.protocol === "https:" ? httpsRequest : httpRequest
  return await new Promise<{ status: number; location: string | null; body: string }>((resolve, reject) => {
    const request = client({
      ...
      response.on("data", (chunk: Buffer) => {
        ...
        if (size > MAX_BYTES) {
          request.destroy(new Error("writer_webfetch_response_too_large"))
          return
        }
    ...
    request.setTimeout(15_000, () => request.destroy(new Error("writer_webfetch_timeout")))
...
export default {
  ...
  async execute(args: { url: string }) {
    ...
    for (let redirects = 0; redirects <= MAX_REDIRECTS; redirects += 1) {
      ...
      const response = await requestPinned(safe.url, safe.hostname, safe.address, safe.family)
      if (response.status >= 300 && response.status < 400) {
        const location = response.location
        if (!location || redirects === MAX_REDIRECTS) throw new Error("writer_webfetch_redirect_limit")
      ...
      return JSON.stringify({ url: safe.url.toString(), status: response.status, content: readableText(response.body) })
    }
