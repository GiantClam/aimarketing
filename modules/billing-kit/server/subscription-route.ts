...
async function reconcilePayPalSubscriptionIfNeeded(localSubscription: Record<string, unknown>) {
  ...
  const remotePlanCode = inferPlanCodeFromRemoteSubscription(
    remoteSubscription,
...
export async function handleBillingSubscriptionGet(request: NextRequest) {
  ...
  try {
    const result = await pool.query(
      `
        SELECT id, enterprise_id, subscribed_by_user_id, plan_code, status, paypal_subscription_id,
               payment_provider, stripe_customer_id, stripe_subscription_id, stripe_checkout_session_id,
               next_plan_code, current_period_start, current_period_end, cancel_at_period_end, created_at, updated_at
    ...
    const workspaceSnapshot = user.enterpriseId ? await getWorkspaceBillingSnapshot(user.enterpriseId) : null
    if (subscription) {
    ...
    const nextWorkspaceSnapshot = user.enterpriseId ? await getWorkspaceBillingSnapshot(user.enterpriseId) : null
    return NextResponse.json({
      subscription: {
...
export async function handleBillingSubscriptionPost(request: NextRequest) {
  const auth = await requireSessionUser(request)
  if ("response" in auth) return auth.response
  const user = (auth as { user: AuthUserPayload }).user
  ...
  try {
    const existingSubscription = await getLatestBillingSubscription(user)
    ...
    if (existingSubscription?.id && existingPayPalSubscriptionId && existingPayPalSubscriptionId === paypalSubscriptionId) {
      const revised = await scheduleSubscriptionPlanChange(existingSubscription.id, plan.code)
      return NextResponse.json({ subscription: revised })
    }

    const result = await savePendingPayPalSubscription({
