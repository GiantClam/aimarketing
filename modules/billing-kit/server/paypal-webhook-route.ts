...
function getSubscriptionId(resource: Record<string, unknown>) {
  return normalizeText(resource.id) || normalizeText(resource.billing_agreement_id) || normalizeText(resource.subscription_id)
}
...
export async function handlePayPalWebhookPost(request: NextRequest) {
