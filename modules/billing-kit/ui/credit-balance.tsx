...
function translateBillingError(code: string, billing: {
  ...
  switch (code) {
    ...
    case "stripe_billing_portal_failed":
      return billing.errorOpenStripePortal
    default:
      return code
  }
}
...
export function CreditBalance({ refreshKey = 0, onSubscriptionLoaded }: CreditBalanceProps) {
  ...
  const canManageStripeSubscription =
    subscription?.subscription?.payment_provider === "stripe" &&
    Boolean(subscription?.subscription?.stripe_customer_id || subscription?.subscription?.stripe_subscription_id)

  const openStripePortal = async () => {
    if (!canManageStripeSubscription || portalLoading) return
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.error || "stripe_billing_portal_failed")
      }
      if (typeof json?.url !== "string" || !json.url) {
        throw new Error("stripe_billing_portal_failed")
      }
      window.location.assign(json.url)
    } catch (portalError) {
      setError(portalError instanceof Error ? portalError.message : "stripe_billing_portal_failed")
    } finally {
      ...
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          {canManageStripeSubscription ? (
            ...
            >
              {portalLoading ? billing.openingStripePortal : billing.manageSubscription}
            </Button>
          ) : null}
