...
- [ ] 5.6 执行 desktop 全量 E2E 与 SaaS lint/build/regression
  ...
  - [x] 2026-08-14 latest real media-only evidence: MiniMax `music-2.6` returned URL audio HTTP 200/schema on the first request; configured non-Seedance `video-minimax-h3` reached RunningHub HTTP 200/schema `SUCCESS` on poll 164; PPTOKEN `gpt-image-2` at `256x256` returned HTTP 200/schema with one image on the latest dedicated and desktop-entry retries; MiniMax `speech-2.8-turbo` remained `Processing` after 120 bounded polls and failed closed; Seedance was excluded.
