...
- [x] 不存在受限主界面或跳过必要 runtime 的路径
  - Evidence (2026-08-14): Desktop bundle-boundary scan reports zero restricted SaaS-only imports/affordances, while native startup ordering and bootstrap readiness tests require WebView2 plus every mandatory runtime component before the main WebView; the 131-test Desktop suite and 39-test Rust suite pass.
...
- [ ] 5.6 执行 desktop 全量 E2E 与 SaaS lint/build/regression
  - [x] 2026-08-14 current Windows rerun passed Desktop tests 131/131, root `pnpm lint`, TypeScript, shared boundary/provenance, Provider parity, AI-entry regressions, desktop build/bundle/Tauri checks, and Next production build (425/425 routes). Full browser E2E and live SaaS regression remain open.
  ...
  - [x] 2026-08-14 `pnpm desktop:test` now passes 131/131, including a built-host offline-egress test that exercises a local file workflow under fetch/http/https/net/tls/dns guards.
  ...
  - [x] 2026-08-14 Desktop i18n regression expanded to unknown Windows/WebView locales (`fr-FR`, `pt-BR`, empty language) and every shared route English label/description; all non-`zh-*` inputs resolve to English and the full Desktop suite passes 131/131.
  ...
  - [x] 2026-08-14 final local gate rerun: `pnpm desktop:test` passed 131/131; `pnpm desktop:build` transformed 1,849 modules; bundle/network scans checked 823,051 bytes with zero violations; Tauri `cargo check`, root lint, Provider parity 2/2, Writer Skill matrix 20/20, Writer cutover check, and SaaS `pnpm build` (425/425 routes) all passed. Clean VM, signed artifacts, and production live-provider/browser gates remain open.
...
- [ ] 除 runtime 源和用户 Provider 外无其他网络请求
  - [x] 2026-08-14 `desktop:verify-network-boundary` scans the built UI text assets (`index.html`, JS and CSS), host/knowledge bundles, runtime manifest tooling and Skill catalog (823,051 UTF-8 bytes) and reports zero hardcoded external endpoints; local loopback and runtime-configured Provider indirection remain allowed. Clean-VM dynamic egress verification remains open.
