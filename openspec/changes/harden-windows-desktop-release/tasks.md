...
- [x] 4.2 生成含 `portable.flag` 的便携 ZIP，全部应用数据位于程序旁 `data/`
  ...
  - [x] 4.2b 2026-08-14 使用最新 release EXE 生成 271,077,590 字节的 `AI-Marketing-Windows-x64-portable.zip`；独立归档读取校验了 EXE、`portable.flag`、README、workflow host、knowledge service、Skill catalog、runtime manifest、安装器和 manifest verifier，以及 EXE、host、knowledge、catalog 的字节长度。跨机器复制验证仍由 4.4 覆盖；NSIS 不作为绿色 ZIP 发布依赖。
  - [x] 2026-08-14 在媒体 Tab 纯状态同步修复后重新执行 `tauri build --no-bundle`，并重新生成当前普通/便携 ZIP；`scripts/verify-desktop-packages.ps1` 通过，分别为 271,011,970 / 271,077,590 字节，SHA-256 为 `db57b0022f704cb9cddd4fcbb4025d79e5a952989a2a9fe00613de6a83aee8c0` / `4fdbd89aacb7c1ee76a378fb7679efd49a6c3b8e4b83b1d6917c61a4c1816f6d`。Windows 主机优先使用 `tar.exe` 生成 ZIP，缺失时回退 `Compress-Archive`。
...
- [ ] 5.6 执行 desktop 全量 E2E 与 SaaS lint/build/regression
  - [x] 2026-08-14 current Windows rerun passed Desktop tests 133/133, root `pnpm lint`, TypeScript, shared boundary/provenance, Provider parity, AI-entry regressions, desktop build/bundle/Tauri checks, and Next production build (425/425 routes). Local Writer fixture E2E also passed. Full browser E2E and live SaaS regression remain deferred by the current green-desktop scope.
  ...
  - [x] 2026-08-14 Desktop i18n regression expanded to unknown Windows/WebView locales (`fr-FR`, `pt-BR`, empty language) and every shared route English label/description; all non-`zh-*` inputs resolve to English and the full Desktop suite passes 133/133.
  ...
  - [x] 2026-08-14 final local gate rerun: `pnpm desktop:test` passed 133/133; `pnpm desktop:build` transformed 1,849 modules; bundle/network scans checked 823,536 bytes with zero violations; Tauri `cargo check`, root lint, Provider parity 2/2, Writer Skill matrix 20/20, local Writer fixture E2E, and root production build (425/425 routes) all passed. Clean VM, signed artifacts, and production live-provider/browser gates remain deferred.
  - [x] 2026-08-14 post-media-tab-sync package gates passed: `desktop:verify-packages` validated normal/portable ZIPs (271,011,970 / 271,077,590 bytes), `desktop:verify-size-budget` stayed within all configured budgets, and `desktop:verify-portable-copy` returned `status=ok` with byte-identical runtime fingerprints and `localAppDataCreated=false`.
  - [x] 2026-08-14 latest real media-only evidence: PPTOKEN `gpt-image-2` at `256x256` and MiniMax music returned HTTP 200/schema on the first request; LLM/image/audio default smoke also passed. Video generation (Seedance and non-Seedance) is excluded from the current release gate.
  - [x] 2026-08-14 latest real media-only evidence supersedes the transient audio queue failures: the default non-video smoke passed LLM/image/audio together, dedicated PPTOKEN `gpt-image-2` (`256x256`) and MiniMax music-only requests each returned HTTP 200/schema on the first attempt. Video generation remained excluded.
  - [x] 2026-08-14 latest local rerun using `apps/desktop/real-providers.test.local.json`: default non-video smoke returned LLM/image/audio HTTP 200/schema; dedicated image-only smoke requested only PPTOKEN `gpt-image-2` at `256x256` and returned HTTP 200/schema on the first attempt; music-only smoke returned HTTP 200/schema on the first attempt. Video generation remained excluded.
  ...
  - [x] 2026-08-14 current release scope explicitly excludes video generation requests; video UI/capability routing and fail-closed task handling remain covered by Desktop contract tests, while real video upstream validation is deferred to manual testing.
