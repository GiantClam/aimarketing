...
- [x] 不存在受限主界面或跳过必要 runtime 的路径
  - Evidence (2026-08-14): Desktop bundle-boundary scan reports zero restricted SaaS-only imports/affordances, while native startup ordering and bootstrap readiness tests require WebView2 plus every mandatory runtime component before the main WebView; the 132-test Desktop suite and 39-test Rust suite pass.
...
- [x] 主程序 ZIP 不重复内置完整 runtime
  - [x] 2026-08-14 `desktop:verify-packages` now fails closed if normal/portable archives contain embedded Python, Node/OpenCode `node_modules`, or a nested `AIMarketing-Runtime-x64.zip`; it now resolves the current `.artifacts/desktop-release/*.zip` outputs before legacy per-mode folders, and the latest release EXE archives pass with 271,011,972 / 271,077,592 compressed bytes.

## 4. Normal and portable packages

- [x] 4.1 生成普通 ZIP，数据/runtime 默认位于 `%LOCALAPPDATA%\AIMarketing`
  - [x] 2026-08-14 使用最新 Windows release EXE 生成并读取校验 `AI-Marketing-Windows-x64-normal.zip`（271,011,972 bytes）；归档含 EXE、host、knowledge service、Skill catalog、runtime manifest、安装器和 manifest verifier，且不含 `portable.flag`。README 标注 `%LOCALAPPDATA%\AIMarketing`、Win10 22H2/Win11 x64 和人工升级方式。
...
- [x] 4.2 生成含 `portable.flag` 的便携 ZIP，全部应用数据位于程序旁 `data/`
  ...
  - [x] 4.2b 2026-08-14 使用最新 release EXE 生成 271,077,592 字节的 `AI-Marketing-Windows-x64-portable.zip`；独立归档读取校验了 EXE、`portable.flag`、README、workflow host、knowledge service、Skill catalog、runtime manifest、安装器和 manifest verifier，以及 EXE、host、knowledge、catalog 的字节长度。跨机器复制验证仍由 4.4 覆盖；NSIS 不作为绿色 ZIP 发布依赖。
  - [x] 2026-08-14 在媒体 Tab 纯状态同步修复后重新执行 `tauri build --no-bundle`，并重新生成当前普通/便携 ZIP；`scripts/verify-desktop-packages.ps1` 通过，分别为 271,011,972 / 271,077,592 字节，SHA-256 为 `bd048f72a570e04610c793c0b9978b95beb7ca31884fe2b5683d9033f519c5e3` / `6fa4fa0e218d6ecedcc5c7b1fd4e2af4399b61c5c9b9bdea5f15c331f2d5fcc5`。Windows 主机优先使用 `tar.exe` 生成 ZIP，缺失时回退 `Compress-Archive`。
...
- [ ] 5.6 执行 desktop 全量 E2E 与 SaaS lint/build/regression
  - [x] 2026-08-14 current Windows rerun passed Desktop tests 132/132, root `pnpm lint`, TypeScript, shared boundary/provenance, Provider parity, AI-entry regressions, desktop build/bundle/Tauri checks, and Next production build (425/425 routes). Full browser E2E and live SaaS regression remain open.
  ...
  - [x] 2026-08-14 `pnpm desktop:test` now passes 132/132, including a built-host offline-egress test that exercises a local file workflow under fetch/http/https/net/tls/dns guards.
  ...
  - [x] 2026-08-14 real Writer validation attempt rebuilt the production app (425/425 routes), completed local demo login, then fail-closed at `/api/writer/availability` with `enabled=false` and `reason=llm_api_key_missing`; Railway OpenCode runtime/R2 production configuration is absent, so fixture E2E is not promoted to production evidence.
  ...
  - [x] 2026-08-14 Desktop i18n regression expanded to unknown Windows/WebView locales (`fr-FR`, `pt-BR`, empty language) and every shared route English label/description; all non-`zh-*` inputs resolve to English and the full Desktop suite passes 132/132.
  - [x] 2026-08-14 Desktop media route action regressions: the shared `/dashboard/video` surface preserves selected audio capabilities, while `/dashboard/ai` and Home reset stale media actions to text; launcher/body feature selection stays synchronized even when the route-level `workflowAction` changes; `routes.test.ts` passes 45/45 and desktop typecheck/build pass.
  - [x] 2026-08-14 model-selector audit: active chat, Writer, workflow, media, Home, and retained Writer migration surfaces all pass the configured model catalog into `ModelControls`; workflow node selection now resolves the matching text/image/video/audio profile before model changes persist; `provider-config.test.ts` and `routes.test.ts` cover configured-model priority, same-capability profiles, and stale-selection fallback.
  - [x] 2026-08-14 final local gate rerun: `pnpm desktop:test` passed 133/133; `pnpm desktop:build` transformed 1,849 modules; bundle/network scans checked 823,536 bytes with zero violations; Tauri `cargo check`, root lint, Provider parity 2/2, Writer Skill matrix 20/20, Writer cutover check, and SaaS `pnpm build` (425/425 routes) all passed. Clean VM, signed artifacts, and production live-provider/browser gates remain open.
  - [x] 2026-08-14 post-media-tab-sync package gates passed: `desktop:verify-packages` validated normal/portable ZIPs (271,011,972 / 271,077,592 bytes), `desktop:verify-size-budget` stayed within all configured budgets, and `desktop:verify-portable-copy` returned `status=ok` with byte-identical runtime fingerprints and `localAppDataCreated=false`.
  - [x] 2026-08-14 latest real media-only evidence: MiniMax `music-2.6` returned URL audio HTTP 200/schema on the first request; configured non-Seedance `video-minimax-h3` reached RunningHub HTTP 200/schema `SUCCESS` on poll 164; PPTOKEN `gpt-image-2` at `256x256` returned HTTP 200/schema with one image on the latest dedicated and desktop-entry retries; the local audio profile now prefers `speech-2.8-hd` with Turbo retained in its model list, a direct HD probe reached `Success` at poll 36, but desktop smoke tasks remained `Processing` through 180 polls and failed closed; Seedance was excluded.
  - [x] 2026-08-14 latest real media-only evidence supersedes the transient audio queue failures: after the smoke payload was aligned with MiniMax's documented async schema, `speech-2.8-hd` reached HTTP 200/schema `Success` on poll 88; the default non-video smoke also passed LLM/image/audio together. Music and non-Seedance H3 video remained successful, PPTOKEN `gpt-image-2` stayed fixed at `256x256`, and Seedance was excluded.
  - [x] 2026-08-14 latest local rerun using `apps/desktop/real-providers.test.local.json`: the local `defaults.video` now points to non-Seedance `video-minimax-h3`; default non-video smoke returned LLM/image/audio HTTP 200/schema with `speech-2.8-hd` terminal `Success`; dedicated image-only smoke requested only PPTOKEN `gpt-image-2` at `256x256` and returned HTTP 200/schema on the first attempt. Seedance remained excluded.
  - [x] 2026-08-14 latest dedicated PPTOKEN image rerun used only `gpt-image-2` at `256x256`; the upstream returned HTTP 200 with `created`/`data`/`usage`, `schemaOk=true`, and one attempt. Video and Seedance were excluded by the smoke scope.
  - [x] 2026-08-14 after one upstream H3 content-audit rejection, the bounded video smoke uses a neutral abstract-geometric prompt; the configured default `video-minimax-h3` then returned RunningHub HTTP 200/schema `SUCCESS` after 116 polls. Seedance remained excluded.
...
- [ ] 除 runtime 源和用户 Provider 外无其他网络请求
  - [x] 2026-08-14 `desktop:verify-network-boundary` scans the built UI text assets (`index.html`, JS and CSS), host/knowledge bundles, runtime manifest tooling and Skill catalog (823,536 UTF-8 bytes) and reports zero hardcoded external endpoints; local loopback and runtime-configured Provider indirection remain allowed. Clean-VM dynamic egress verification remains open.
