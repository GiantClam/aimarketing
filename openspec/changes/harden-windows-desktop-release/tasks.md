...
- [ ] 5.6 执行 desktop 全量 E2E 与 SaaS lint/build/regression
  - [x] 2026-08-14 current Windows rerun passed Desktop tests 120/120, root `pnpm lint`, TypeScript, shared boundary/provenance, Provider parity, AI-entry regressions, desktop build/bundle/Tauri checks, and Next production build (425/425 routes). Full browser E2E and live SaaS regression remain open.
  ...
  - [x] 2026-08-14 `pnpm desktop:test` now passes 120/120, including a built-host offline-egress test that exercises a local file workflow under fetch/http/https/net/tls/dns guards.
  ...
  - [x] 2026-08-14 latest real media-only evidence: configured MiniMax audio reached HTTP 200/schema `Success` on poll 12 with a 60-poll bounded budget; configured non-Seedance `video-minimax-h3` reached RunningHub HTTP 200/schema `SUCCESS` on poll 106; PPTOKEN `gpt-image-2` at `256x256` remains unavailable and Seedance was excluded.
