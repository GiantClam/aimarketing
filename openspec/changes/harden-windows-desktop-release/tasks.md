...
- [ ] 5.6 执行 desktop 全量 E2E 与 SaaS lint/build/regression
  ...
  - [x] 2026-08-14 latest real media-only evidence: configured MiniMax audio reached HTTP 200/schema `Success` on poll 12 with a 60-poll bounded budget; MiniMax `music-2.6` returned URL audio HTTP 200/schema on the first request; configured non-Seedance `video-minimax-h3` reached RunningHub HTTP 200/schema `SUCCESS` on poll 106; PPTOKEN `gpt-image-2` at `256x256` returned HTTP 200/schema with one image on the latest dedicated and desktop-entry retries; Seedance was excluded.
