...
- [x] 4.3 删除 desktop bundle 中 login、registration、tenant、role、balance、subscription、Agent publishing、market 和 enterprise preset affordances
  - Evidence (2026-08-14): `apps/desktop/test/routes.test.ts` scans the desktop source and shared route manifest for account/billing/publishing/enterprise-preset markers; the 132-test Desktop suite passes and `pnpm desktop:verify-bundle` reports zero violations across the built UI/host/knowledge bundles.
...
- [x] 4.6 添加 streaming、长工具输出、取消、重启、artifact 和缺配置 UI tests — 2026-08-14 `apps/desktop/test/workbench-client.test.ts`, `opencode-serve.test.ts`, `routes.test.ts`, `rpc.test.ts`, and `media-runtime.test.ts` cover streamed text/tool output, oversized frames, cancellation, crash/restart recovery, artifact routing, and configuration-required UI states; the current Desktop suite passes 132/132.
...
- [x] 6.2 用一个真实配置 Provider 运行普通对话 smoke。✓ 2026-08-13 — `apps/desktop/real-providers.test.local.json` 驱动的 `test:real-providers` 已确认 LLM `/chat/completions` HTTP 200（choices/usage）；同次图片请求连续 3 次 HTTP 502（上游 `upstream_error`，已记录为外部阻塞）；视频/seedance 按验证范围明确未执行。
  ...
  - [x] 2026-08-14 latest image revalidation supersedes the transient timeout records: `/v1/models` returned HTTP 200 with 15 entries including `gpt-image-2`; the dedicated smoke and the desktop `test:real-providers:image` entry point each used only `gpt-image-2` at `256x256` and returned HTTP 200/schema with one image on a successful attempt. The proxy's `401 Invalid token` did not prevent the direct route from satisfying the one-success gate; video and Seedance remained excluded.
  - [x] 2026-08-14 current media revalidation keeps ordinary chat independent of provider queue state: music-only `music-2.6` passed HTTP 200/schema, video-only selected non-Seedance `video-minimax-h3` and reached `SUCCESS` after 164 polls, while the local audio profile prefers `speech-2.8-hd` (with Turbo retained as a switchable model) and desktop audio tasks stayed `Processing` after 120/180 polls. A direct HD probe reached `Success` at poll 36; no Seedance request was made.
  - [x] 2026-08-14 audio smoke request contract was aligned with the official MiniMax async schema; the configured HD audio profile then reached `Success` on poll 88, and the default non-video smoke passed LLM/image/audio together (`gpt-image-2` at `256x256`, audio `Success` at poll 63) with video and Seedance excluded.
...
- [x] 测试和诊断证据完整 — Desktop 132/132, OpenCode serve/session/RPC, boundary/provenance, typecheck/build, and Writer browser fixture/provider-missing evidence are recorded; clean VM and production live-provider gates remain in hardening.
