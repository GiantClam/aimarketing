...
- [ ] 3.3 Extract host-neutral OpenCode session/event handling from the Railway runtime without sharing the Railway HTTP server.
...
**Quality Gate:** PASSED for the contract slice — shared parser tests, existing seven protocol tests, package typechecks, root lint, and boundary scan pass. Provider normalization and host session extraction remain open in 3.3–3.4.
...
- [ ] 4.1 Copy graph, migration, foreach/collect, cancellation, retry, and resume tests into `workflow-core` and verify they fail before implementation is exported.
- [ ] 4.2 Extract schema, node definitions, connection validation, definition migrations, and plan compilation.
- [ ] 4.3 Introduce capability, repository, artifact, event, clock, and cancellation ports.
- [ ] 4.4 Extract execution and iteration behavior without enterprise IDs, credit fields, Next responses, or database record types.
...
**Quality Gate:** PASSED for the schema/registry/connect slice — workflow-core typecheck and 3 contract tests pass. Definition migrations, compiler, execution, and SaaS adapter remain open.
