...
- [x] 5.4 Convert shared AI Entry message rendering and workflow UI slices to injected clients and navigation. ✓ 2026-08-13 — the shared cloud message shell, typed WorkbenchClient ports and injected navigation now cover the migrated AI Entry, workflow, desktop history and artifact UI slices; host-only rich SaaS state remains in the Web adapter/composition layer.
...
- [x] 5.4b SaaS AI Entry route replacement now accepts an injected `NavigationAdapter`; the Next router is retained only as its host-composition fallback.
  ...
  - [x] 5.4e Desktop workbench startup, deep-link history and retry preparation now consume the stable injected `Desktop WorkbenchClient` conversation port instead of directly invoking local message/list commands. ✓ 2026-08-13
  - [x] 5.4f `WorkbenchClient` now carries a typed workflow list/save port; Desktop workflow startup, save, import and execution persistence consume its local IPC adapter, while the Web adapter maps the existing SaaS workflow API. ✓ 2026-08-13
  - [x] 5.4g Desktop media, asset-library, and fallback artifact reveal actions now consume the injected `WorkbenchClient.files.reveal` port; only the local adapter owns the `open_artifact` IPC mapping. ✓ 2026-08-13
...
- [x] 6.2 Add contract tests that run equivalent fixtures through the shared core and SaaS adapters. ✓ 2026-08-13 — ordinary DAG and completed-output retry fixtures assert identical typed terminal output across the shared core and SaaS adapter.
  ...
  - [x] 6.2b Equivalent retry fixture asserts both paths consume the persisted completed output and produce the same retried typed bundle. ✓ 2026-08-13
...
- [x] 6.6 Mark this change ready only when all parity gates pass and downstream desktop changes can consume stable exports. ✓ 2026-08-13 — shared boundary/provenance scans, shared-core and SaaS-adapter parity fixtures, desktop regression suite, root TypeScript, strict lint and Next production build all pass against the stable shared exports.
