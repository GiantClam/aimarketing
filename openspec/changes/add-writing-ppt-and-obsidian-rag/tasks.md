...
- [x] 连续两轮修改保持同一项目/session 上下文
  - [x] 2026-08-14 persisted OpenCode session IDs now flow through the shared `WorkbenchConversation` contract, Desktop Tauri conversation adapter, startup state restoration, and subsequent `session.create`; route/workbench regressions plus current `pnpm desktop:test` (127/127) and `pnpm desktop:build` pass.
...
- [x] 全部新增测试、SaaS build 和 desktop build 通过 — 2026-08-14 current Desktop 127/127, desktop typecheck/build, root lint, root typecheck, and Next production build all passed.
