...
- [x] 9.3 Run the complete Writer unit, integration, route, billing, recovery, runtime, Skill, TypeScript, and ESLint suites.
  - [x] 2026-08-14 Writer Skill/contract, asset, asset-runtime, revision, UI, session recovery, root TypeScript and ESLint suites all pass; current Desktop 131/131, shared boundary/provenance checks, and both Writer browser scenarios pass. Browser/production smoke gates remain separately open.
...
- [ ] 9.8 Run production smoke tests for actual persistent OpenCode execution, complete active-draft revision, real Skill activation/release/digest, title preservation, URL research, cover generation, multi-image completion, revision conflicts, and billing idempotency.
  - [x] 2026-08-14 local real-browser harness rebuilt the SaaS app (425/425 routes) but correctly stopped at the availability gate with `reason=llm_api_key_missing`; no production Writer pass was claimed and the missing environment credential remains an external prerequisite.
