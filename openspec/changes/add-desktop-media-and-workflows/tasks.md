...
- [x] 每类真实 Provider smoke 结果已记录
  ...
  - 2026-08-14 latest media-only revalidation with the current local config: MiniMax `music-2.6` returned HTTP 200/schema on the first request; non-Seedance `video-minimax-h3` returned RunningHub HTTP 200/schema `SUCCESS` after 164 bounded polls; MiniMax `speech-2.8-turbo` returned HTTP 200 but remained `Processing` after 120 bounded polls and therefore failed closed. Seedance remained excluded.
