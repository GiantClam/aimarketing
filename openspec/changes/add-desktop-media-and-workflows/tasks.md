...
- [x] 所有阶段与质量门禁通过 — 2026-08-14 Desktop 131/131, desktop build, bundle-boundary scan, root TypeScript and lint pass; real-provider scope remains LLM/image/audio plus the separately verified non-Seedance video profile, with Seedance intentionally excluded.
...
- [x] 每类真实 Provider smoke 结果已记录
  ...
  - 2026-08-14 latest PPTOKEN revalidation supersedes the timeout record for the current upstream: `/v1/models` returned HTTP 200 with 15 entries including `gpt-image-2`; the dedicated `--mode=both` connectivity smoke sent only `gpt-image-2` at `256x256`, direct generation returned HTTP 200/schema with one image in 29.9s, and the proxy returned `401 Invalid token`. Because the gate accepts any one successful configured path, the smoke returned `success=true`; no other image model, video, or Seedance was executed and no credential was emitted.
  - 2026-08-14 immediate desktop-entry retry after the transient timeout used the same local config, `--image-only`, `AIMARKETING_PROVIDER_IMAGE_SIZE=256x256`, and `AIMARKETING_PROVIDER_RETRIES=0`: the configured PPTOKEN `gpt-image-2` request returned HTTP 200/schema on attempt 1, with `scope.executed=[image]`, `scope.excluded=[video,seedance]`, and no credential output.
  - 2026-08-14 latest media-only revalidation with the current local config: MiniMax `music-2.6` returned HTTP 200/schema on the first request; non-Seedance `video-minimax-h3` returned RunningHub HTTP 200/schema `SUCCESS` after 164 bounded polls; the local audio profile now prefers configured `speech-2.8-hd` and retains `speech-2.8-turbo` in its model list, but desktop smoke tasks remained `Processing` after 120/180 bounded polls and failed closed. A direct HD model probe completed with HTTP 200/schema `Success` at poll 36; Seedance remained excluded.
  - 2026-08-14 latest audio recovery after aligning the smoke request with MiniMax's documented async payload (`language_boost` plus `audio_sample_rate`/`bitrate`/`format`/`channel`): configured `speech-2.8-hd` returned HTTP 200/schema `Success` on poll 88 in the isolated audio smoke, and the default non-video smoke passed LLM HTTP 200/schema, `gpt-image-2` at `256x256` HTTP 200/schema, and audio HTTP 200/schema `Success` on poll 63. Scope remained `executed=[llm,image,audio]`, `excluded=[video,seedance]`.
