...
function buildRuntimeModelConfig(modelId: string): ModelConfig | null {
  if (!modelId.startsWith("openrouter:")) return null
...
export async function POST(req: Request) {
  try {
    ...
    const modelConfig =
      allModels.find((m) => m.id === model) || buildRuntimeModelConfig(model)
