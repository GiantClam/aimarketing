...
import { duplicateCustomAgent } from "@/lib/platform/custom-agents"
...
export async function POST(
  ...
  try {
    ...
    const data = await duplicateCustomAgent({
    ...
    const message = error instanceof Error ? error.message : "custom_agent_duplicate_failed"
    const status = message === "custom_agent_not_found" ? 404 : 500
