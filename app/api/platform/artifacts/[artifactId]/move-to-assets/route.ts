...
import { getPlatformArtifact, movePlatformArtifactToAssetLibrary } from "@/lib/platform/task-run-store"
...
export async function POST(
  ...
  try {
    ...
    const result = await movePlatformArtifactToAssetLibrary(artifact.id, artifact.enterpriseId)
    ...
    return NextResponse.json({
      data: {
        ...
        deletedWorkItemIds: result.deletedWorkItemIds,
      },
    })
  } catch (error) {
    const message = error instanceof Error ? error.message : "move_artifact_to_asset_library_failed"
    const status =
