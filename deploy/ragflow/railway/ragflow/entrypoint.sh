...
function probe_postgres() {
    [[ "${DB_TYPE}" == "postgres" ]] || return 0

    echo "Probing PostgreSQL connectivity to ${POSTGRES_HOST}:${POSTGRES_PORT:-5432}/${POSTGRES_DBNAME:-postgres} as ${POSTGRES_USER} sslmode=${POSTGRES_SSLMODE:-require}"
    "$PY" - <<'PY'
import os
...
try:
    ...
    row = cur.fetchone()
    print(f"PostgreSQL probe OK: current_user={row[0]}")
    cur.close()
    conn.close()
except Exception as ex:
    print(f"PostgreSQL probe failed: {type(ex).__name__}: {ex}")
    raise
PY
}
...
if [[ "${INIT_MODEL_PROVIDER_TABLES}" -eq 1 ]]; then
    if [[ "${DB_TYPE}" == "postgres" ]]; then
        echo "Skipping model provider table migrations for DB_TYPE=postgres; bundled script is MySQL-specific."
    else
