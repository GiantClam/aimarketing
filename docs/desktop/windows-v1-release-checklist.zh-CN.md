...
| 范围 | 证据 |
| --- | --- |
| Desktop 单元/集成回归 | `pnpm desktop:test`：129/129 |
...
| 真实 Provider（当前状态） | 最新本机重跑（配置 `apps/desktop/real-providers.test.local.json`）：默认视频 profile 已切到非 Seedance `video-minimax-h3`；默认非视频 smoke 的 LLM/image/audio 均 HTTP 200/schema，MiniMax `speech-2.8-hd` 最终 `Success`；专用图片 smoke 固定只请求 PPTOKEN `gpt-image-2`、优先 `256x256`，HTTP 200/schema、首个请求成功；默认 H3 视频 smoke 经 116 次轮询后 RunningHub `SUCCESS`。历史 music-only 证据仍保留；Seedance 未执行。 |
...
下面分为两类：Desktop 本地绿色发布门禁，以及线上共享核心的独立生产验证。Railway Writer 不属于 Desktop 本地 OpenCode 的运行依赖；它只决定是否可以额外宣称线上 SaaS Writer 的生产一致性。
...
- 线上共享 Writer 独立生产门禁：URL research、完整修订、图片恢复、质量盲测、生产 OpenCode Skill release/digest、billing idempotency 尚未完成；本机 fixture 浏览器 E2E 已通过但不替代生产验证。2026-08-14 真实验证尝试已完成本地生产构建（425/425 routes）和 demo 登录，但 `/api/writer/availability` 返回 `enabled=false`、`reason=llm_api_key_missing`；当前环境没有 Railway OpenCode runtime/R2 生产配置，未伪报通过。该项不阻塞 Desktop 本地 OpenCode 的构建、打包和离线运行，但阻止宣称线上 SaaS Writer 生产一致性已完成。
...
- 最新非 Seedance video-only 结果：`video-minimax-h3` HTTP 200/schema、116 次轮询后 `SUCCESS`；Seedance 仍明确排除。
