...
| 范围 | 证据 |
| --- | --- |
| Desktop 单元/集成回归 | `pnpm desktop:test`：117/117 |
...
| 真实 Provider | 默认 smoke（`apps/desktop/real-providers.test.local.json`，图片优先 `256x256`）：LLM HTTP 200/schema（1 次）、`gpt-image-2` HTTP 200/schema（2 次）、MiniMax audio HTTP 200/schema（第 5 次轮询 `Success`）；脱敏输出为 `scope.executed=[llm,image,audio]`、`scope.excluded=[video,seedance]`。最终 video-only smoke 仅选择非 Seedance `video-minimax-h3`，RunningHub HTTP 200/schema、122 次轮询后 `SUCCESS`；Seedance 明确未执行 |
