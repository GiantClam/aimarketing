...
- MiniMax 音频 provider：历史一次 60 次预算曾在第 12 次达到 `Success`，但当前连续两次重验分别在 60/120 次有界轮询后仍为 `Processing`；当前不能宣称音频 gate 通过，需上游任务队列恢复后重新运行。
