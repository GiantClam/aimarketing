...
| 真实 Provider | 最新默认 smoke（`apps/desktop/real-providers.test.local.json`，图片优先 `256x256`）：LLM HTTP 200/schema（1 次）、`gpt-image-2` HTTP 200/schema（2 次）、MiniMax audio HTTP 200/schema（第 5 次轮询 `Success`）；脱敏输出为 `scope.executed=[llm,image,audio]`、`scope.excluded=[video,seedance]`。非 Seedance 视频历史 smoke：RunningHub MiniMax-Hailuo-H3 HTTP 200、已记录 106/133 次轮询后 `SUCCESS`；Seedance 明确未执行 |
