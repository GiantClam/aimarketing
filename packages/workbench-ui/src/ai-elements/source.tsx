...
export function MessageResponse({ content, children, className, streaming = false, parseIncompleteMarkdown = streaming, components, ...props }: HTMLAttributes<HTMLDivElement> & { content?: string; streaming?: boolean; parseIncompleteMarkdown?: boolean; components?: Components }) { return <div {...props} className={cx("ai-elements-message-response", "message-body", className)} data-slot="message-response">{children ?? <Streamdown mode={streaming ? "streaming" : "static"} parseIncompleteMarkdown={parseIncompleteMarkdown} animated={streaming ? { animation: "fadeIn", duration: 120, easing: "ease-out", sep: "word", stagger: 24 } : false} isAnimating={streaming} components={components} remarkPlugins={[remarkGfm]}>{content ?? ""}</Streamdown>}</div>; }
