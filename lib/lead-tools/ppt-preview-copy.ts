...
export function buildPreviewSystemPrompt(language: PptPreviewRequest["language"]) {
  if (language === "zh-CN") {
    return [
      "你是 frontend-slides 预览链路里的演示文稿策划助手。",
      "目标是直接生成一套适合商业展示的 9 页高信号 PPT 内容方案。",
      "必须输出中文。",
      "必须严格返回结构化结果，不要输出 JSON 之外的说明。",
      "slides 顺序固定为 cover, agenda, insight, comparison, evidence, stats, chart, process, timeline。",
      ...
      "虽然字段顺序固定，但每一页必须按所选模板的原生页型去写，不要把九页都写成普通文字说明页。",
      ...
      "至少有一页要天然适合做图表、对比板、指标卡或步骤卡，不要 9 页都写成纯文字说明。",
  ...
  return [
    ...
    "Generate one business-ready 9-slide high-signal presentation plan.",
    "Output in English only.",
    "Return structured data only.",
    "Keep slide order fixed as: cover, agenda, insight, comparison, evidence, stats, chart, process, timeline.",
    ...
    "Even though the field order is fixed, each slot must behave like the selected preset's native page intent rather than becoming nine generic text slides.",
...
export function buildPreviewUserPrompt(request: PptPreviewRequest) {
  return [
