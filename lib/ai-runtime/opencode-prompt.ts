    ...
    ...(isBusinessAgent
      ...
      ? [
          ...
          input.projectSnapshot
            ? "Before generating, read ./.runtime/project-snapshot.json and reconstruct the current project state in ./workspace/ppt-master. If state.source is runtime-workspace, restore each state.files entry to its relative path under ./workspace before continuing. Treat the snapshot as structured application state only; do not expect it to contain images, SVG, PPTX, logs, caches, or other process files."
