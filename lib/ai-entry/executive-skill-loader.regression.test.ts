...
test("ppt assistant skill documents are loadable through the shared agent loader", async () => {
  ...
  assert.match(presentationContent, /native `\/opt\/dashiai-ppt\/SKILL\.md` skill/i)
  assert.match(presentationContent, /Work conversationally across turns/i)
  assert.match(presentationContent, /Do not call the legacy PPT preview\/export\/brief tools/i)
})
