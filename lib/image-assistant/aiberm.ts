...
const IMAGE_MODEL_HIGH = process.env.IMAGE_ASSISTANT_AIBERM_MODEL || "gemini-3.1-flash-image-preview"
...
export async function generateOrEditImages(params: {
  ...
  if (shouldUseImageAssistantFixtures()) {
    return {
      ...
      textSummary: "已生成本地 fixture 图片结果。",
  ...
  return {
    provider: "aiberm",
    model: result.model,
    textSummary: result.textSummary || "已生成新的图片候选结果。",
    images: expanded,
  }
}
