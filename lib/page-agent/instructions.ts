...
- data-agent-add-node="<节点类型>"：节点库里的按钮，点击即在画布添加该类型节点。类型取值：upload, text_input, file_create, writer, llm_generate, agent_execute, image_generate, video_generate, digital_human, music_generate, voice_synthesis, audio_generate, ppt_generate, knowledge_retrieve, knowledge_write, product_store。
...
- data-agent-config="<nodeKey>:<字段>"：节点配置里的输入框/文本域。可填写的文本字段例如 text_input 的 :text、llm_generate 的 :systemPrompt、agent_execute 的 :prompt、digital_human 的 :script 与 :scenePrompt。下拉选择保持默认即可。
...
- data-agent-add-node="<type>": library buttons that add a node of that type to the canvas. Types: upload, text_input, file_create, writer, llm_generate, agent_execute, image_generate, video_generate, digital_human, music_generate, voice_synthesis, audio_generate, ppt_generate, knowledge_retrieve, knowledge_write, product_store.
...
- data-agent-config="<nodeKey>:<field>": inputs/textareas in a node's config. Text fields you may fill include text_input's :text, llm_generate's :systemPrompt, agent_execute's :prompt, digital_human's :script and :scenePrompt. Leave dropdown selects at their defaults.
...
text_input(文本输入) → llm_generate(模型生成脚本) → voice_synthesis(语音合成 TTS) → digital_human(口播数字人)。
...
text_input (text input) → llm_generate (LLM generates the spoken script) → voice_synthesis (TTS) → digital_human (talking digital human).
