    ...
    await pool.query(
      `
        ...
        WHERE (enterprise_code = $1 OR name = $2)
          AND NOT EXISTS (
            SELECT 1 FROM enterprises other WHERE other.enterprise_code = $3 AND other.id <> enterprises.id
          );
      `,
...
async function ensureEnterprise(pool, enterprise) {
  const inserted = await pool.query(
    `
      INSERT INTO enterprises (enterprise_code, name, updated_at)
...
async function upsertEnterpriseDifyBinding(pool, enterpriseId) {
  ...
  const inserted = await pool.query(
    `
      INSERT INTO aimarketing_enterprise_dify_bindings (enterprise_id, base_url, api_key, enabled, created_at, updated_at)
...
async function resolveDifyApiKey(pool) {
  ...
  throw new Error("ENTERPRISE_DIFY_API_KEY is required when no legacy Dify API key is available in dify_connections")
}

async function replaceDatasets(pool, bindingId, enterprise) {
  await pool.query(`DELETE FROM aimarketing_enterprise_dify_datasets WHERE binding_id = $1`, [bindingId])
  await pool.query(
    `
      INSERT INTO aimarketing_enterprise_dify_datasets (
        binding_id,
        dataset_id,
        dataset_name,
        scope,
      ...
      VALUES ($1, $2, $3, 'brand', 10, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    `,
    [bindingId, enterprise.datasetId, enterprise.datasetName],
  )
}
  ...
  try {
    ...
    const missingTables = await pool.query(`
      SELECT ARRAY_REMOVE(ARRAY[
        CASE WHEN to_regclass('public.enterprises') IS NULL THEN 'enterprises' END,
        CASE WHEN to_regclass('public.aimarketing_enterprise_dify_bindings') IS NULL THEN 'aimarketing_enterprise_dify_bindings' END,
        CASE WHEN to_regclass('public.aimarketing_enterprise_dify_datasets') IS NULL THEN 'aimarketing_enterprise_dify_datasets' END
      ], NULL) AS names;
    `)
