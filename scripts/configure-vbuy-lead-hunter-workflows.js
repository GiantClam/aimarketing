...
async function main() {
  ...
  try {
    ...
    // Remove the legacy alias row to avoid duplicated semantics.
    await pool.query(
      `
        DELETE FROM "${TABLES.advisorConfigs}"
        WHERE enterprise_id = $1
          AND advisor_type = 'lead-hunter';
      `,
      [enterprise.id],
    )
    ...
    const savedRes = await pool.query(
      `
        SELECT advisor_type, base_url, api_key, enabled
        FROM "${TABLES.advisorConfigs}"
        WHERE enterprise_id = $1
          AND advisor_type IN ('company-search', 'contact-mining')
