...
ARTIFACT_DIR = ROOT / "artifacts" / "image-assistant" / "canvas-reference-regression"
    ...
    with log_path.open("w", encoding="utf-8") as log_file:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "scripts/image_assistant_canvas_reference_regression_e2e.py"],
                cwd=str(ROOT),
                check=False,
                env={
                    **base_env,
                    "IMAGE_ASSISTANT_TEST_BASE_URL": base_url,
                    "IMAGE_ASSISTANT_TEST_SESSION_ID": os.environ.get("IMAGE_ASSISTANT_TEST_SESSION_ID", "").strip(),
                },
            )
