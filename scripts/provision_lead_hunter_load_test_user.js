...
function parseArgs(argv) {
  const args = {
    email: `lead-hunter-load-${Date.now()}@example.com`,
    password: "TempLoadTest123!",
    enterpriseCode: "vbuy",
    name: "Lead Hunter Load Test",
    role: "admin",
  }
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      if (!userId) {
        throw new Error("load_test_user_upsert_failed")
      }

      for (const featureKey of FEATURE_KEYS) {
        await upsertPermission(client, userId, featureKey, featureKey === "expert_advisor")
      }
