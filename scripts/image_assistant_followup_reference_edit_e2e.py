...
SCENARIO = os.environ.get("IMAGE_ASSISTANT_E2E_SCENARIO", "followup-reference-edit").strip()
...
def run():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            followup_payload = page.evaluate(
                """async ({ baseUrl, sessionId, prompt, parentVersionId }) => {
                    const response = await fetch(`${baseUrl}/api/image-assistant/generate`, {
                      method: "POST",
                      credentials: "include",
            ...
            expect(
                bool(followup_payload.get("ok")),
                f"follow-up generate request failed: {followup_payload}",
            )
            ...
            result["steps"].append(
                {
                    ...
                    "request_url": str(followup_payload.get("url") or ""),
                    "task_id": followup_task_id or None,
                    "direct": bool(followup_data.get("direct")),
                    "transport": "api_direct_generate_route",
                }
            )
