...
function chooseAgentId(agentPayload) {
  const agents = Array.isArray(agentPayload?.agents) ? agentPayload.agents : []
  if (agents.length === 0) {
    throw new Error("ai_entry_agents_empty")
  }
  ...
  const allIds = agents
    ...
    .filter(Boolean)
  if (allIds.length === 0) {
    throw new Error("ai_entry_agents_missing_id")
  }

  const preferred = allIds.find((id) => id === "executive-diagnostic")
  if (preferred) return preferred

  const defaultAgentId =
    typeof agentPayload?.defaultAgentId === "string"
      ? agentPayload.defaultAgentId.trim()
      : ""
  if (defaultAgentId && allIds.includes(defaultAgentId)) {
...
async function run() {
  ...
  const agentsRes = await requestJson(baseUrl, "/api/ai/agents", { method: "GET" }, cookieHeader)
  if (!agentsRes.ok) {
    throw new Error(`ai_entry_agents_failed:${agentsRes.status}:${agentsRes.bodyText.slice(0, 200)}`)
  }
  const selectedAgentId = chooseAgentId(agentsRes.payload)
  ...
  const returnedAgentId =
    typeof chatRes.payload?.agentId === "string" ? chatRes.payload.agentId.trim() : ""
  if (returnedAgentId && returnedAgentId !== selectedAgentId) {
    throw new Error(`ai_entry_agent_mismatch:${selectedAgentId}:${returnedAgentId}`)
  }
  ...
  const summary = {
    ...
    selectedAgentId,
    returnedAgentId: returnedAgentId || null,
    fallbackAttemptCount: attemptErrors.length,
    answerPreview: answer.slice(0, 120),
  }

  console.log("AI_ENTRY_AGENT_SELECTION_SMOKE_TEST_START")
  console.log(JSON.stringify(summary, null, 2))
  console.log("AI_ENTRY_AGENT_SELECTION_SMOKE_TEST_END")
}

run().catch((error) => {
  console.error("ai_entry_agent_selection_smoke_test.failed", error instanceof Error ? error.message : String(error))
  process.exitCode = 1
})
