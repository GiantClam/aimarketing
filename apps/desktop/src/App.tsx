...
function DesktopWriterWorkspace({ route, prompt, onPromptChange, runStatus, activeRunId, onRun, onCancel, activePrompt, activePromptAt, assistantText, assistantAt, toolEvents, artifacts, onArtifactOpen, model, models, reasoningEffort, skillId, onModelChange, onReasoningChange, onSkillChange }: {
  route: DesktopRoute;
  prompt: string;
  onPromptChange: (value: string) => void;
  runStatus: string;
  ...
  return (
    <div className="chat-canvas flex h-full min-h-0 justify-center">
      <section className="chat-workspace-section writer-cloud-workspace">
        ...
        {previewOpen ? <div className="writer-preview-overlay" role="dialog" aria-modal="true"><section className="writer-preview-sheet"><header><div><div className="dashboard-kicker">{writerCopy.preview}</div><h2>{writerCopy.finalPreview}</h2><p className="writer-preview-description">{writerCopy.previewHint}</p></div><button type="button" className="ghost" onClick={() => setPreviewOpen(false)}>{writerCopy.close}</button></header>{previewEditing ? <textarea className="writer-preview-editor" autoFocus value={previewDraft} onChange={(event) => setPreviewDraft(event.target.value)} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === "Enter") { event.preventDefault(); commitPreviewDraft(); } if (event.key === "Escape") { event.preventDefault(); setPreviewDraft(assistantText); setPreviewEditing(false); } }} /> : <WorkbenchWriterMessage role="assistant" label={writerCopy.assistant} content={assistantText} timestamp={assistantAt} artifacts={runArtifacts} onArtifactOpen={onArtifactOpen} />}<div className="writer-preview-actions"><button type="button" className="dashboard-button-secondary" onClick={() => { if (previewEditing) commitPreviewDraft(); else setPreviewEditing(true); }} disabled={!assistantText}>{previewEditing ? writerCopy.done : writerCopy.edit}</button><button type="button" className="dashboard-button-primary" onClick={() => void copyText("rich")}>{copyKind === "rich" ? writerCopy.copied : writerCopy.rich}</button><button type="button" className="dashboard-button-secondary" onClick={() => void copyText("markdown")}>{copyKind === "markdown" ? writerCopy.copied : writerCopy.markdown}</button><button type="button" className="dashboard-button-secondary" onClick={() => { setPreviewOpen(false); onGenerateImages?.(); }}>{writerCopy.generateImageWithCopy}</button><button type="button" className="ghost" onClick={() => setPreviewOpen(false)}>{writerCopy.done}</button></div></section></div> : null}
      </section>
    </div>
  );
}
  ...
  return <div ref={viewportRef} className={`workflow-canvas-viewport ${dragging ? "is-dragging" : ""}`} onPointerDown={startPan} onPointerMove={movePointer} onPointerUp={endPointer} onPointerCancel={endPointer}>
    ...
    <div className="workflow-canvas-scene" style={{ transform: `translate(${viewport.x}px, ${viewport.y}px) scale(${viewport.scale})` }}>
      ...
      {nodes.map((node, index) => <button key={node.nodeKey} type="button" className={`workflow-node workflow-canvas-node ${selectedNodeKey === node.nodeKey ? "workflow-node-active" : ""}`} style={{ left: node.positionX, top: node.positionY }} onPointerDown={(event) => startNodeDrag(event, node)} onClick={() => onSelectNode(node.nodeKey)}>
        <span>{index + 1}</span><strong>{nodeTitle(node)}</strong><small>{node.type}</small>{!providerConfiguredForNode(node.type) && requiresConfiguredProviderForWorkflowAction(node.type) ? <em>{locale === "en" ? "Configuration required" : "需要配置"}</em> : null}<i aria-hidden="true" />
      </button>)}
    </div>
