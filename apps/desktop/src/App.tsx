...
function DesktopMediaWorkspace(props: DesktopMediaWorkspaceProps) {
  ...
  useEffect(() => {
    const nextFeatureId = isVideo ? ((Object.entries(actionByFeature).find(([, action]) => action === workflowAction)?.[0] as MediaFeatureId | undefined) ?? "text-to-video") : null;
    setActiveFeatureId(nextFeatureId);
    if (!nextFeatureId) { setOpenFeatureIds([]); return; }
    setOpenFeatureIds((current) => current.includes(nextFeatureId) ? current : [...current, nextFeatureId]);
  }, [isVideo, workflowAction]);
  ...
  const closeFeature = (featureId: MediaFeatureId) => {
    const next = openFeatureIds.filter((id) => id !== featureId);
    setOpenFeatureIds(next);
    if (activeFeatureId !== featureId) return;
    const nextActive = next.at(-1) ?? null;
    setActiveFeatureId(nextActive);
    const nextAction = nextActive ? actionByFeature[nextActive] : undefined;
    if (nextAction) onWorkflowAction(nextAction);
  };
...
export function App() {
  ...
  const activeCapability = capabilityForWorkflowAction(selected.path === "/dashboard/video" || selected.path === "/dashboard/workflows" ? workflowAction : routeAction ?? "llm_generate");
