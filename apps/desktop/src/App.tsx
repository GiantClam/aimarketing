...
function DesktopLibraryWorkspace({ route, artifactRows, savedWorkflows, conversations, runs, taskCount, tokenCount, artifactCount, providerCost: initialProviderCost, estimatedCost: initialEstimatedCost, onNavigate, onRetryRun, onArtifactRemove, onArtifactReveal, knowledgeQuery, knowledgeResults, knowledgeStatus, onKnowledgeQueryChange, onKnowledgeSearch, locale }: { route: DesktopRoute; artifactRows: Array<ArtifactRow>; savedWorkflows: Array<{ id: string; name: string; definition_json: string; updated_at: string }>; conversations: Array<{ id: string; title: string; updated_at: string }>; runs: RunRow[]; taskCount: number; tokenCount: number; artifactCount: number; providerCost?: number; estimatedCost?: number; onNavigate: (path: string) => void; onRetryRun: (run: RunRow) => void; onArtifactRemove: (artifactId: string) => void; onArtifactReveal: (relativePath: string, mimeType: string) => void; knowledgeQuery: string; knowledgeResults: KnowledgeResult[]; knowledgeStatus: string; onKnowledgeQueryChange: (value: string) => void; onKnowledgeSearch: () => void; locale: "zh" | "en" }) {
  const [providerCost, setProviderCost] = useState(initialProviderCost);
  const [usageCost, setUsageCost] = useState(initialEstimatedCost);
  useEffect(() => { setProviderCost(initialProviderCost); setUsageCost(initialEstimatedCost); void tauriBridge.invoke<{ provider_cost?: number | null; estimated_cost?: number | null }>("usage_summary").then((summary) => { setProviderCost(typeof summary.provider_cost === "number" ? summary.provider_cost : undefined); setUsageCost(typeof summary.estimated_cost === "number" ? summary.estimated_cost : undefined); }).catch(() => undefined); }, [initialEstimatedCost, initialProviderCost]);
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setProviderCost(summary.provider_cost);
