...
function buildWorkflowDefinition(prompt: string, actionId: WorkflowAction, provider: DesktopConfig["provider"], extraConfig: Record<string, unknown> = {}): WorkflowDefinitionEnvelope {
...
function DesktopWorkflowWorkspace({ route, prompt, onPromptChange, runStatus, activeRunId, onRun, onCancel, savedWorkflows, workflowAction, onWorkflowAction, definition, onDefinitionChange, onSave, onExport, onImport, locale }: DesktopWorkflowWorkspaceProps & { locale: "zh" | "en" }) {
...
function DesktopAssetLibrarySurface({ artifactRows, onArtifactRemove, locale }: { artifactRows: ArtifactRow[]; onArtifactRemove: (artifactId: string) => void; locale: "zh" | "en" }) {
...
export function App() {
  ...
  const reasoningEffort = config.provider.reasoningEffort ?? "auto";
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (stored) {
          ...
          const runtimeChanged = selectedRuntime.opencodePath !== stored.runtime.opencodePath || selectedRuntime.pythonPath !== stored.runtime.pythonPath || selectedProvider.model !== stored.provider.model;
        ...
        if (activeConfig) {
          try {
            ...
            if (activeConfig.obsidianVaultPath && activeConfig.obsidianIndexPath) {
              await tauriBridge.invoke("host_send", { message: { version: 1, requestId: `vault-startup-${Date.now()}`, type: "knowledge.index", payload: { vaultPath: activeConfig.obsidianVaultPath, indexPath: activeConfig.obsidianIndexPath, embedding: { baseUrl: "http://127.0.0.1:11434", model: "nomic-embed-text" } } } });
            }
              ...
              await tauriBridge.invoke("host_send", { message: {
                ...
                payload: {
                  ...
                  config: {
                    ...
                    model: typeof payload.model === "string" ? payload.model : activeConfig.provider.model,
                    baseUrl: activeConfig.provider.baseUrl,
                    apiKey: activeConfig.provider.apiKey,
                    endpoint: activeConfig.provider.endpoint,
                    queryEndpoint: activeConfig.provider.queryEndpoint,
                  },
                },
              } });
    ...
    void tauriBridge.listen<{ raw: string }>("desktop://runtime-response", (payload) => {
      try {
        ...
        const frame = JSON.parse(payload.raw.slice(separator + 1)) as { requestId?: string; ok?: boolean; data?: { sessionId?: string; event?: { event?: string; delta?: string; runId?: string; inputTokens?: number; outputTokens?: number; costUsd?: number } } };
        ...
        if (event?.runId) {
          ...
          if (eventType === "usage") { setTokenCount((current) => current + (event.inputTokens ?? 0) + (event.outputTokens ?? 0)); if (typeof event.costUsd === "number") setEstimatedCost((current) => current + event.costUsd!); void tauriBridge.invoke("record_usage", { runId: event.runId, model: configRef.current.provider.model || "unknown", inputTokens: event.inputTokens ?? null, outputTokens: event.outputTokens ?? null, estimatedCost: event.costUsd ?? null, idempotencyKey: `${event.runId}:usage:${sequence}` }); }
  ...
  async function rebuildVaultIndex() {
    ...
    try {
      ...
      const response = await sendHostMessage({ version: 1, requestId, type: "knowledge.index", payload: { vaultPath: config.obsidianVaultPath, indexPath: config.obsidianIndexPath, embedding: { baseUrl: "http://127.0.0.1:11434", model: "nomic-embed-text" } } });
  ...
  async function searchKnowledge() {
    ...
    try {
      await tauriBridge.invoke("host_start");
      const response = await sendHostMessage({ version: 1, type: "knowledge.search", payload: { indexPath: config.obsidianIndexPath, query, limit: 8, embeddingBaseUrl: "http://127.0.0.1:11434", embeddingModel: "nomic-embed-text" } });
  ...
  async function runAgent(promptOverride?: string, mediaFeatureId?: MediaFeatureId | "image_generate", mediaInputs?: Record<string, unknown>, workflowOverride?: unknown) {
    ...
    try {
      ...
      const capabilityConfig = {
        ...
        queryEndpoint: config.provider.queryEndpoint,
      ...
      if (usesOpenCodeConversation) {
        ...
        const sessionResponse = await sendHostMessage({ version: 1, requestId: `${conversationId}:session`, type: "session.create", payload: { conversationId, ...(existingSessionId ? { sessionId: existingSessionId } : {}), workspacePath: config.workspacePath, model: config.provider.model, provider: config.provider } });
  ...
  return (
    <div className="shell" style={{ "--wb-background": WORKBENCH_THEME.light.background, "--wb-foreground": WORKBENCH_THEME.light.foreground, "--wb-card": WORKBENCH_THEME.light.card, "--wb-primary": WORKBENCH_THEME.light.primary, "--wb-primary-foreground": WORKBENCH_THEME.light.primaryForeground, "--wb-muted": WORKBENCH_THEME.light.muted, "--wb-muted-foreground": WORKBENCH_THEME.light.mutedForeground, "--wb-border": WORKBENCH_THEME.light.border, "--wb-body-font": WORKBENCH_THEME.typography.body, "--wb-display-font": WORKBENCH_THEME.typography.display, "--wb-message-max-width": WORKBENCH_MESSAGE_FRAME.maxWidth, "--wb-message-padding": WORKBENCH_MESSAGE_FRAME.rowPadding, "--wb-message-gap": WORKBENCH_MESSAGE_FRAME.gap, "--wb-message-avatar-size": WORKBENCH_MESSAGE_FRAME.avatarSize, "--wb-message-avatar-radius": WORKBENCH_MESSAGE_FRAME.avatarRadius } as CSSProperties}>
      ...
      <section className={`workspace ${selected.path === "/dashboard" ? "workspace-home" : ""} ${immersivePage ? "workspace-immersive" : ""}`.trim()}>
        ...
        {selected.path === "/dashboard" ? <div className="home-shell"><div className="home-page-shell"><header className="home-topbar"><div className="home-topbar-status"><span className="public-signal" />{homeCopy.workspaceReady}{savedWorkflows.length ? <span className="home-topbar-count">{savedWorkflows.length} {locale === "zh" ? "个流程可用" : "flows ready"}</span> : null}</div><button className="home-credits-link" onClick={() => navigate("/dashboard/tasks")}><span className="home-credits-icon"><WorkbenchRouteIcon name="sparkles" size={14} /></span><span>{homeCopy.viewUsage}</span><WorkbenchRouteIcon name="arrowUpRight" size={15} /></button></header><main className="home-main">
