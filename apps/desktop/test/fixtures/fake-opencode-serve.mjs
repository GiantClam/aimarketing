...
const server = createServer(async (request, response) => {
  ...
  if (stabilityMode) {
    const stabilityMatch = url.pathname.match(/^\/session\/([^/]+)\/(message|abort)$/u);
    if (stabilityMatch) {
      ...
      if (request.method === "POST" && action === "message" && stabilitySessions.has(sessionId)) {
        let body = "";
        ...
        setTimeout(() => {
          ...
          sendEvent({ payload: { type: "message.updated", properties: { sessionID: sessionId, info: { id: messageId, role: "assistant" } } } });
  ...
  if (request.method === "POST" && url.pathname === "/session/recovered-session/message") {
    ...
    setTimeout(() => {
      ...
      sendEvent({ payload: { type: "message.updated", properties: { ...sessionProperty, info: { id: "assistant-1", role: "assistant" } } } });
      ...
      sendEvent({ payload: { type: "message.part.updated", properties: { ...sessionProperty, part: { id: "tool-1", messageID: "assistant-1", type: "tool", tool, state: { status: "completed", title: "saved" } } } } });
